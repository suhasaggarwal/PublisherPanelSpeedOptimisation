...
public class AggregationModule
{
    ...
    throws Exception
  {
	  ...
	  PrintStream out = new PrintStream(new FileOutputStream("subcategorydaily.txt"));
	  System.setOut(out);
	  setUp();
	  updateCookieSubcategoryCountDaily("2016-11-18","2016-11-18");
  ...
  public static void updateCookieSubcategoryCountDaily(String startdate, String enddate)
		    throws CsvExtractorException, Exception
		  {
				String query0 = "SELECT COUNT(*)as count,cookie_id,subcategory FROM enhanceduserdatabeta1 where date between " + "'" + startdate + "'" + " and " + "'" + enddate + "' group by cookie_id,subcategory limit 20000000";  
				CSVResult csvResult0 = getCsvResult(false, query0);
			    List<String> headers0 = csvResult0.getHeaders();
			    List<String> lines0 = csvResult0.getLines();
			    List<PublisherReport> pubreport0 = new ArrayList(); 
			    ...
			    */
			       ...
			       subcategoryMap.put(subcategory, count00);   
			       ...
			       fingerprintDetails = null;
				    ...
				    for (SearchHit hit : fingerprintDetails) {
					    ...
					    audience_Segment = (String)result.get("subcategory");
//Update count corresponding to each Audience Segment
					    audience_Segment = audience_Segment.trim();
					    if(audience_Segment.equals(subcategory)){
					    count00 = subcategoryMap.get(audience_Segment);
				     ...
				     //   result.put("date",enddate);
				        IndexCategoriesData1.updateDocument(client, "subcategoryindex1", "core2", documentId, "count", finalCount.toString());
				      //  IndexCategoriesData1.updateDocument(client, "subcategoryindex1", "core2", documentId, "date", enddate);
				        foundflag=1;
					    }
				        subcategoryMap.clear();
				        ...
				        if(foundflag==0)
				        {
					    	 ...
					    	 if(data0[0]!=null && data0[1]!=null && data0[2]!=null && data0[0].isEmpty()==false && data0[1].isEmpty()==false && data0[2].isEmpty()==false){
					    	        ...
					    	        subcategorydata.put("subcategory",data0[1]);
					    	        subcategorydata.put("cookie_id",data0[0]);
					    	        subcategorydata.put("count",data0[2]);
