...
public class AggregationModule
{
    ...
    throws Exception
  {
	  ...
	  PrintStream out = new PrintStream(new FileOutputStream("audiencesegmentdaily.txt"));
	  System.setOut(out);
	  setUp();
	  updateCookieAudienceSegmentCountDaily("2016-11-18","2016-11-18");
	   ...
	   //     String[] data = ((String)lines.get(i)).split(",");
	     ...
	     //   if(data[0].trim().toLowerCase().contains("festivals"))
	      ...
	      //  else
	        ...
	        if ((!entry.getKey().equals("tech")) && (!entry.getKey().equals("india")) && (!entry.getKey().trim().toLowerCase().contains("foodbeverage")) )
	        {
  ...
  public static void updateCookieAudienceSegmentCountDaily(String startdate, String enddate)
		    throws CsvExtractorException, Exception
		  {
				String query0 = "SELECT COUNT(*)as count,cookie_id,audience_segment FROM enhanceduserdatabeta1 where date between " + "'" + startdate + "'" + " and " + "'" + enddate + "' group by cookie_id,audience_segment limit 20000000";  
				CSVResult csvResult0 = getCsvResult(false, query0);
			    List<String> headers0 = csvResult0.getHeaders();
			    List<String> lines0 = csvResult0.getLines();
			    List<PublisherReport> pubreport0 = new ArrayList(); 
			    ...
			    */
		           ...
		           count00 = data0[2].toString();
			       
			       audienceSegment = data0[1];
			       ...
			       audienceSegmentMap.clear();
			       
			       audienceSegmentMap.put(audienceSegment, count00);   
			       ...
			       fingerprintDetails = null;
				    ...
				    fingerprintDetails = ElasticSearchAPIs.searchDocumentFingerprintIds(client,
								"audiencesegmentindex1", "core2",
								"cookie_id", fingerprintId);
				
				//  if(fingerprintId.equals("0fd018b4_bbda_434d_bc1e_548bdb4a3782"))
				//    {
...
//Update count corresponding to each Audience Segment
					    audience_Segment = audience_Segment.trim();
					    
					    if(audience_Segment.equals(audienceSegment)){
					    count00 = audienceSegmentMap.get(audience_Segment);
				    ...
				    //    result.put("date",enddate);
				        IndexCategoriesData1.updateDocument(client, "audiencesegmentindex1", "core2", documentId, "count", finalCount.toString());
				    //    IndexCategoriesData1.updateDocument(client, "audiencesegmentindex1", "core2", documentId, "date", enddate);
				        foundflag=1;
						}
				        audienceSegmentMap.clear();
				      }
				    ...
				    if(foundflag==0){
				    if(data0[0]!=null && data0[1]!=null && data0[2]!=null && data0[0].isEmpty()==false && data0[1].isEmpty()==false && data0[2].isEmpty()==false){
					    	       
				    	           data0[1] = data0[1].trim();
				    	           audienceSegmentdata.put("audience_segment",data0[1]);
					    	       audienceSegmentdata.put("cookie_id",data0[0]);
					    	       audienceSegmentdata.put("count",data0[2]);
